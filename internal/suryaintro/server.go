...
// Description: This file contains the gRPC server implementation for the suryaintro
// API defined in api/suryaintro.proto. This implementation is used in the
...
// Joke finds a joke that hasn't been told in this organization before.
func (s *Server) Joke(ctx context.Context) (string, error) {
	...
	conn, err := s.cfg.ConnMap["suryaintro"].Writer(ctx, uuid.UUID(org))
	if err != nil {
		return "", err
	}

	res, err := conn.ExecContext(ctx, `
        INSERT INTO joke (joke, last_told) VALUES ($1, now()) ON CONFLICT DO NOTHING
    `, text)
	if err != nil {
		return "", err
	}
